...
public class UserServiceTest {
    ...
    void testUnbanUser_UserNotFound() {
        ...
        assertThrows(UserNotFoundException.class, () -> userService.unbanUser(userId));
    }

    @Test
    void testGetUsersByBlockedStatus() {
        when(userRepository.findAllByBlocked(true)).thenReturn(List.of(user));

        List<User> blockedUsers = userService.getUsersByBlockedStatus(true);

        assertEquals(1, blockedUsers.size());
        assertEquals(user, blockedUsers.get(0));
        verify(userRepository).findAllByBlocked(true);
    }
